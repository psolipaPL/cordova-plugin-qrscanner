...
class QRScanner : CDVPlugin, AVCaptureMetadataOutputObjectsDelegate {
        ...
        do {
            if (captureSession?.isRunning != true){
                ...
                if(!allowedFormats.isEmpty){
                    ...
                    for i in 0 ..< allowedFormats.count {
                        switch allowedFormats[i] {
                            case "QR_CODE":
                                metaOutput!.append(AVMetadataObject.ObjectType.qr)
                                break;
                            case "EAN_13":
                                metaOutput!.append(AVMetadataObject.ObjectType.ean13)
                                break;
                            case "ITF":
                                metaOutput!.append(AVMetadataObject.ObjectType.itf14)
                                break;
                            case "CODE_128":
                                metaOutput!.append(AVMetadataObject.ObjectType.code128)
                                break;
                        }
                    }
                }else{
                    metaOutput!.metadataObjectTypes = [AVMetadataObject.ObjectType.qr, AVMetadataObject.ObjectType.ean13, AVMetadataObject.ObjectType.itf14, AVMetadataObject.ObjectType.code128]
                }
    ...
    func metadataOutput(_ captureOutput: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        if (found.type == AVMetadataObject.ObjectType.qr || found.type == AVMetadataObject.ObjectType.ean13 || found.type == AVMetadataObject.ObjectType.itf14 || found.type == AVMetadataObject.ObjectType.code128) && found.stringValue != nil {
            scanning = false
    ...
    @objc func configs(_ command: CDVInvokedUrlCommand){
        let formats = command.arguments[0] as! String
        allowedFormats = formats.components(separatedBy: ",");
    }
