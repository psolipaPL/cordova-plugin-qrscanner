...
public class QRScanner extends CordovaPlugin implements BarcodeCallback {
    ...
    private ArrayList<String> formats = null;
    ...
    private void configs(String[] allowedFormats args){
        String[] tmp = allowedFormats.split(",");
        List<String> tmpList = Arrays.asList(tmp);
        formats = new ArrayList<String>(tmpList);
    }
    ...
    private void setupCamera(CallbackContext callbackContext) {
        cordova.getActivity().runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if(!formats.isEmpty()){
                    int iterator = 0;
                    while(formats.size() > iterator){
                        switch(formats.get(iterator)){
                            case 'QR_CODE':
